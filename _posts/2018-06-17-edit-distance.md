...
**Solution**  
Wagner–Fischer 알고리즘은 교체, 삭제, 삽입의 연산을 기반으로 서로 다른 두 문자열 이 동일해 질 수 있는 최소 거리를 계산한다. 이러한 최소 거리를 편집 거리라 부른다. 주로 NLP(자연어처리) 프로그램에서 자연어와 사전의 단어간 유사도를 측정하여 맞춤법 교정 후보를 선출하는데 사용된다.  
